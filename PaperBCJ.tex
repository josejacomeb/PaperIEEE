...
This reduction permitted to reduce the number of parameters to 1651376(it's a 25\% of parameters than the original net), and what about the precision of the net, it was slighty affected but it reach a 97.2\% of accuracy.\\
...
So, the complete network model is formed as follows: the input layer where an RGB image 64x64 pixels, followed by a convolution layer of 16 filters, a kernel of 5 and a stride of 1 pixel, this layer  obtaining only the main characteristics of the image, then a pooling layer is applied, which has the function to find the maximum values of the previous layer with a kernel size and stride of 2, then again a second layer of convolution is applied that has 16 filters, size of kernel 5 and a stride of 1, followed by a new layer of pooling with a kernel size and stride of 2, so the result image was has size of 16x16x16 and finally the net is completed with three fully-connected layers, the first with 384 neurons, the next with 192 and the last with 2 neurons which represent the both classes  the layers of type fully-connected with a total of 478 neurons, where the last two neurons that is the output represent the two classes to classify, the result model is shown in the next graph.\\
...
In the graph of training shown below, we can appreciate the final result of the net accuracy, 9000 iterations were neccesary to reduce the Loss value to its minimun and the architecture shows that it can reach an acceptable accuracy with few iterations. Note that by reducing the number of filters the network gained accuracy because the dataset does not have so much features learn, so the net found a better solution with a less number of parameters.      \\
...
The results shown that classification time by GPU get worse by 66\%, mainly due OpenBLAS make extense use of CPU Threads causing that not all the parameters of the net were computed by GPU like 
