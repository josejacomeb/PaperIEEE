...
Aunque  los resultados de la red son excelentes,una de las propuestas de este trabajo es la implementacion en tiempo real,por lo que el numero de parametros a ser procesados sera importante, por lo que se conisdero modificar la red para mejorar el tiempo de procesamiento, para lograr esto fue encesario reducir el n\'umero de filtros de la cNET de 64 hasta 16.\\	
...
Esto permitió reducir el número de parámetros a 1651376 (es un 25 \% de parámetros que la red original),donde la precisión de la red, fue ligeramente afectada pero alcanza un 97.2 \% de exactitud.\\

This permitted to reduce the number of parameters to 1651376(it's a 25\% of parameters than the original net), where the precision of the net, it was slighty affected but it reach a 97.2\% of accuracy.\\

El modelo completo de la red queda formado de la siguiente manera: la capa de entrada donde ingresa una imagen RGB 64x64 pixeles, seguida por una capa de convolucion de 5 kernels y un stride de 1 píxel, capa que reduce la imagen a un de tamaño de 16x32x32 obteniendo solamente sus características principales, luego se aplica una capa de pooling, que tiene la funcion de encontrar los valores máximos de la capa anterior mediante 16 filtros, dejando un tamaño de 32x32x48, entonces nuevamente se aplica una segunda capa de convolucion que tiene 5 kernels y un stride de 1, logrando un tamaño de 32x32x16 seguida por una nueva  capa de pooling con 16 filtros de tamaño 32x16x16 y finalmente las capas de tipo fully-connected con un total de 478 neuronas, donde las dos ultimas neuronas que estana la salida representan las dos clases establecidas en esta red.\\
...
The follow picture shows the schematic of the cNET of 16 filters and its process of training.Note that by reducing the number of filters the network gained accuracy because it does not have so much data to process.\\
	...
	\section{Test}
Para esta etapa se consideró un conjunto de datos adicional con 200 imágenes de plantas de maíz en estadio de V3-V7 y 203 imágenes de malezas, obtenidas en cultivos de maíz, los resultados fueron comparados entre cNET con 64 y 16 filtros en la Tabla\\
...
Como se muestra en la tabla, los resultados alcanzados por el procesador GPU, es la mejor opci\'on para clasificar una gran cantidad de imágenes sin embargo este proyecto se piensa que es port\'atil, por lo que consideramos el RaspBerry Pi para el an\'alisis de rendimiento. Con cNET de 64 filtros los resultados para el tiempo de clasificaci\'on fue de 615 milisegundos, por lo que el tiempo no es el \'optimo para nuestro objetivo que es el procesamiento en tiempo real.\\
