...
\end{comment}	
Now to compare all these results taking into account that an image captured by the camera at a height of 1.5 m can focus an approximate 6 plants of maize, and according to the amount of weed, an average of 12 plants, Giving a total of about 18 plants. This analyzes how many frames per second(fps) would be appropriate for the acquisition of images.
\begin{table}[h!]
\centering
\begin{tabular}[c c c c]{|p{1.1 cm}|p{1.8cm}|p{1.8cm}|p{1.9cm}|} 
 \hline
 \textbf{Parameter} &\textbf{GPU(Without Batching) } & \textbf{CPU with Batching} & \textbf{RaspBerry Pi 3 with Batching} \\ 
 \hline 
 \textbf{Time(s)} & 0.0171 & 0.196 & 2.714  \\ [0.95ex]
 \hline
 \textbf{FPS} & 58.47  & 5.08 & 0.36  \\ [0.95ex]
 \hline
\end{tabular}
\caption{Test of comparation with the frames per second of the different processors}
\label{table:5}
\end{table}
