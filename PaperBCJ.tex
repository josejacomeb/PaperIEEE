...
Once the network architecture was chosen, it is constituted as follows: the input, a 64x64 RGB image layer followed by a convolution layer which reduces the image by taking only its main characteristics, then a layer of pooling which finds the maximum values of the characteristics found in the previous layer, followed by a second convolution, and a second layer of pooling, and finally two layers of type fully connected, from which in the end we have the output representing the two classes established in this network.\\
